-- Auto-generated installer: Mech redstone computer (v0.9)
...
print('Installing: Mech redstone computer (v0.9)')
for path, data in pairs(files) do
  local dir = path:match('^(.*)/[^/]+$')
  if dir and not fs.exists(dir) then fs.makeDir(dir) end
  local h = fs.open(path, 'w'); h.write(b64dec(data)); h.close(); print('  + ' .. path)
end
print(''); print('Done! Type: reboot')
